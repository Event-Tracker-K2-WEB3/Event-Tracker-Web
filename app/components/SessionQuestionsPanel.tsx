...
export default function SessionQuestionsPanel({
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      console.error(err);
      setError("Unable to submit the question.");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  async function handleUpvote(questionId: number) {
    try {
      ...
      console.error(err);
      setError("Unable to add your vote.");
    }
  }
  ...
  return (
    <aside className="premium-card-hover animate-fade-up animation-delay-200 rounded-3xl border border-white/10 bg-[#0b1020]/80 p-6 shadow-[0_0_45px_rgba(15,23,42,0.45)]">
      <div className="mb-7 flex items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-white">Live Questions</h2>
          <p className="mt-1 text-sm text-white/45">
            Top-voted questions appear first.
          </p>
        </div>
      ...
      {live ? (
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 text-sm leading-7 text-white/55">
          The questions section will only be available when this session
          is live.
        </div>
      )}
