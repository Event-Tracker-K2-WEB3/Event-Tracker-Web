...
export function FavoriteButton({ event }: { event: Event }) {
    ...
    useEffect(() => {
        ...
        const checkFavoriteStatus = () => {
            ...
            if (saved) {
                const parsed = JSON.parse(saved) as FavoriteSession[];
                startTransition(() => {
    ...
    const toggleFavorite = (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        const saved = localStorage.getItem('eventsync_favorites');
        let currentFavorites: FavoriteSession[] = saved ? JSON.parse(saved) : [];
        ...
        if (exists) {
            ...
            currentFavorites.push({
                ...
                startTime: new Date(event.startDate).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }),
                endTime: new Date(event.endDate).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }),
                room: event.location
            });
            setIsFavorite(true);
        }
    ...
    return (
        ...
        >
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill={isFavorite ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2" className="size-5">
