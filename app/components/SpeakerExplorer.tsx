  ...
  return (
    <div className="min-h-screen bg-[#080812] text-white overflow-x-hidden">
      ...
      <section className="relative py-20 px-6 overflow-hidden">
        ...
        {/* Effet Waouh : Le conteneur monte et passe de opacity 0 à 100 */}
        <div className={`relative max-w-7xl mx-auto text-center transition-all duration-1000 ease-out transform ${
          isMounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
        }`}>
          <span className="inline-block text-violet-400 text-sm font-medium tracking-widest uppercase mb-4 animate-pulse">
            Nos intervenants
          </span>
          <h1 className="text-5xl md:text-6xl font-bold mb-6 bg-gradient-to-b from-white to-white/70 bg-clip-text text-transparent tracking-tight">
          ...
          <p className="mt-4 text-violet-400 text-sm font-medium">
            {speakers.length} intervenant{speakers.length > 1 ? "s" : ""} confirmé{speakers.length > 1 ? "s" : ""}
          </p>
        </div>
      </section>
      ...
      <section className="px-6 pb-8">
        {/* Effet Waouh : Arrive un poil plus tard (duration-1000, delay-150) */}
        <div className={`max-w-7xl mx-auto bg-gradient-to-r from-[#0d0d17]/80 via-[#120e25]/70 to-[#0d0d17]/80 backdrop-blur-xl border border-violet-500/20 rounded-2xl p-5 shadow-[0_8px_32px_0_rgba(109,40,217,0.1)] flex flex-col xl:flex-row items-end gap-5 justify-between transition-all duration-1000 delay-150 ease-out transform ${
      ...
      <section className="px-6 pb-16">
        <div className="max-w-7xl mx-auto">
          {affiches.length === 0 ? (
            <div className="text-center py-24 text-white/30 animate-fade-in">
              <p className="text-lg">Aucun intervenant ne correspond à votre recherche.</p>
            </div>
          ) : (
            <>
              <p className={`text-white/30 text-sm mb-6 transition-all duration-700 delay-300 ${
                isMounted ? "opacity-100" : "opacity-0"
              }`}>
                {filtres.length} intervenant{filtres.length > 1 ? "s" : ""} trouvé{filtres.length > 1 ? "s" : ""}
              </p>
              
              {/* Le conteneur des cartes s'anime de manière globale d'abord */}
              ...
              {visible < filtres.length && (
                <div className="mt-12 text-center">
                  <button
                    ...
                    className="px-8 py-3 rounded-xl border border-violet-500/40 text-violet-400 text-sm font-medium hover:bg-violet-500/10 hover:border-violet-400 hover:scale-[1.02] active:scale-[0.98] transition-all duration-200"
                  >
